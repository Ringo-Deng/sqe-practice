...
export function TextbookBookmarkEditor({draft,bookTitle,controller,onChange,onClose,pageUnit='页'}:{
 ...
 return <>
  <Dialog open onOpenChange={open=>{if(!open&&!controller.busy)onClose();}}>
   <DialogContent className="textbook-note-editor" showCloseButton={!controller.busy}>
    <DialogHeader><DialogTitle>{saved?'编辑书签':'添加书签'}</DialogTitle><DialogDescription>{bookTitle} · 第 {draft.page} {pageUnit}</DialogDescription></DialogHeader>
    <form onSubmit={save}>
