...
export function TextbookLibrary({controller,catalog,bookmarks,guest=false,initial,kind='textbook',initialPanel=null}:{controller:TextbookAnnotationsController;catalog:TextbookCatalogController;bookmarks:TextbookBookmarksController;guest?:boolean;initial?:TextbookReaderTarget;kind?:'textbook'|'mindmap';initialPanel?:LibraryPanel|null}){
 ...
 const filterBooks=useMemo(()=>catalog.books.filter(item=>filter==='all'||(filter==='mine'?item.imported:subjectById(item.subjectId)?.group===filter)),[catalog.books,filter]);
 const subjectFilters=useMemo(()=>[
  {id:'all',label:'全部科目'},
  ...subjects.filter(subject=>(filter!=='FLK1'&&filter!=='FLK2')||subject.group===filter).map(subject=>({id:subject.id,label:subject.zh})),
 ...
 const filteredBooks=useMemo(()=>{
  ...
  return filterBooks.filter(item=>{
   ...
   return (subjectFilter==='all'||item.subjectId===subjectFilter)&&(!term||`${item.title} ${item.shortTitle} ${subject?.zh??'未分类'} ${subject?.en??''}`.toLocaleLowerCase().includes(term));
  });
 ...
 return <section className="textbook-library-page textbook-library-refined textbook-library-compact" onKeyDown={event=>{
  ...
  <div className={`textbook-library-layout ${panel??'reading'}-open`} style={{'--textbook-toolbar-height':`${toolbarHeight}px`} as CSSProperties}>
   <aside id="textbook-catalog-panel" className="textbook-catalog" aria-label="教材目录" hidden={panel!=='catalog'}>
    ...
    <div className="textbook-catalog-filters" role="group" aria-label="教材分类">{bookFilters.filter(item=>!isMindMap||item.id!=='mine').map(item=><button key={item.id} type="button" aria-pressed={filter===item.id} onClick={()=>{setFilter(item.id);setSubjectFilter('all');}}>{item.label}</button>)}</div>
     ...
     {filteredBooks.length?<section aria-label="书目列表">{filteredBooks.map(item=>{
      ...
      return <div className={`textbook-catalog-row ${isCurrent?'active':''}`} key={item.id}>
       <button className="textbook-catalog-select" onClick={()=>selectBook(item.id)} aria-current={isCurrent?'page':undefined}>
        ...
        <small title={page?`上次读到第 ${page} 页，共 ${item.pageCount} 页`:`共 ${item.pageCount} 页`}>{page?`${page}/${item.pageCount}`:item.pageCount} {isMindMap?'段':'页'}</small>
       </button>
       {!isMindMap&&<Button size="icon" variant="ghost" aria-label={`编辑 ${item.shortTitle}`} title="修改书名与科目" disabled={catalog.busy} onClick={()=>openEdit(item)}><Pencil size={14}/></Button>}
      </div>;
   ...
   <aside id="textbook-notes-panel" className="textbook-note-index" aria-label="教材高亮笔记" hidden={panel!=='notes'}>
    ...
    {controller.loading&&!controller.data?<div className="textbook-notes-loading"><Loader2 className="animate-spin" size={18}/>正在读取…</div>:!bookNotes.length?<div className="textbook-notes-empty"><div className="textbook-note-empty-icon"><Highlighter size={24}/></div><p>把重要的规则留下来</p><small>在{isMindMap?'导图':'教材'}中选中文字，即可高亮或写笔记。<br/>保存后，在这里点击即可回到原文。</small></div>:!filteredNotes.length?<div className="textbook-filter-empty"><p>没有匹配的笔记</p><Button variant="ghost" size="sm" onClick={()=>setNoteQuery('')}>查看全部笔记</Button></div>:<div className="textbook-note-list">{filteredNotes.map(item=><button key={item.id} onClick={()=>jumpToPage(item.bookId,item.page)}><span>第 {item.page} {isMindMap?'段':'页'}<span>回到原文 →</span></span><blockquote>{item.quote}</blockquote>{item.note&&<p>{item.note}</p>}</button>)}</div>}
   </aside>
   <aside id="textbook-bookmarks-panel" className="textbook-note-index textbook-bookmark-index" aria-label="教材书签" hidden={panel!=='bookmarks'}>
    ...
    {bookmarks.loading&&!bookmarks.data?<div className="textbook-notes-loading"><Loader2 className="animate-spin" size={18}/>正在读取…</div>:!bookBookmarks.length?<div className="textbook-filter-empty"><Bookmark size={24}/><p>{bookmarks.error?'暂时无法读取书签':'还没有书签'}</p></div>:!filteredBookmarks.length?<div className="textbook-filter-empty"><p>没有匹配的书签</p><Button variant="ghost" size="sm" onClick={()=>setBookmarkQuery('')}>查看全部书签</Button></div>:<div className="textbook-bookmark-list">{filteredBookmarks.map(item=><div className={`textbook-bookmark-row ${item.page===target.page?'active':''}`} key={`${item.bookId}:${item.page}`}><button type="button" className="textbook-bookmark-jump" onClick={()=>jumpToPage(item.bookId,item.page)} aria-current={item.page===target.page?'page':undefined}><span><Bookmark size={14}/>第 {item.page} {isMindMap?'段':'页'}</span>{item.note&&<p>{item.note}</p>}</button><Button variant="ghost" size="icon" aria-label={`编辑第 ${item.page} 页书签备注`} title="编辑备注" disabled={bookmarks.busy} onClick={()=>setBookmarkDraft(item)}><Pencil size={14}/></Button></div>)}</div>}
   </aside>
  </div>
  {bookmarkDraft&&<TextbookBookmarkEditor pageUnit={isMindMap?'段':'页'} key={`${bookmarkDraft.bookId}:${bookmarkDraft.page}:${bookmarkDraft.revision??'new'}`} draft={bookmarkDraft} bookTitle={catalog.books.find(item=>item.id===bookmarkDraft.bookId)?.shortTitle??'教材'} controller={bookmarks} onChange={setBookmarkDraft} onClose={()=>setBookmarkDraft(null)}/>}
