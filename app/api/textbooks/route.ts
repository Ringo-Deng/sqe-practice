...
import type {TextbookCatalogData} from '@/lib/textbook-catalog';
...
type TitleRow={bookId:string;title:string};
type ImportedRow={id:string;title:string;originalName:string;pageCount:number;sizeBytes:number;createdAt:number;revision:number};

function cleanTitle(value:unknown){return typeof value==='string'?value.trim().replace(/\s+/g,' ').slice(0,120):'';}
...
async function payload(userId:string):Promise<TextbookCatalogData>{
 ...
 const [titleResult,importedResult]=await Promise.all([
  db.prepare('SELECT book_id AS bookId,title FROM textbook_titles WHERE user_id=? ORDER BY updated_at DESC').bind(userId).all<TitleRow>(),
  db.prepare('SELECT id,title,original_name AS originalName,page_count AS pageCount,size_bytes AS sizeBytes,created_at AS createdAt,revision FROM user_textbooks WHERE user_id=? ORDER BY created_at DESC,id').bind(userId).all<ImportedRow>()
 ]);
 const titles=Object.fromEntries(titleResult.results.filter(row=>!!textbookById(row.bookId)).map(row=>[row.bookId,row.title]));
 const imported:Textbook[]=importedResult.results.map(row=>({id:row.id,title:row.title,shortTitle:row.title,subjectId:'my-materials',version:'导入 PDF',pageCount:row.pageCount,url:`/api/textbooks/file?id=${encodeURIComponent(row.id)}`,sha256:`user-${row.id}`,pageUrlTemplate:'',imported:true,originalName:row.originalName,sizeBytes:row.sizeBytes,createdAt:row.createdAt}));
...
export async function POST(request:Request){
 ...
 if(contentType.startsWith('application/pdf')){
  const url=new URL(request.url),title=cleanTitle(url.searchParams.get('title')),pageCount=Number(url.searchParams.get('pageCount')),fileName=cleanFileName(url.searchParams.get('fileName')),declaredSize=Number(url.searchParams.get('size'));
 ...
 try{const text=await request.text();if(text.length>2000)throw Error();body=JSON.parse(text);if(!body||typeof body!=='object'||Array.isArray(body))throw Error();}catch{return json({error:'教材名称格式无效。'},400);}
 if(body.action!=='rename'||typeof body.id!=='string')return json({error:'未知操作。'},400);
 const title=cleanTitle(body.title);if(!title)return json({error:'教材名称不能为空。'},400);
 try{
  const db=database(),now=Date.now(),builtIn=textbookById(body.id);
  if(builtIn){
   await db.prepare('INSERT INTO textbook_titles (user_id,book_id,title,updated_at) VALUES (?,?,?,?) ON CONFLICT(user_id,book_id) DO UPDATE SET title=excluded.title,updated_at=excluded.updated_at,revision=textbook_titles.revision+1').bind(user.userId,builtIn.id,title,now).run();
  }else{
   const result=await db.prepare('UPDATE user_textbooks SET title=?,updated_at=?,revision=revision+1 WHERE id=? AND user_id=?').bind(title,now,body.id,user.userId).run();
   if(!result.meta.changes)return json({error:'没有找到这本教材，请刷新后重试。'},404);
  }
  return json(await payload(user.userId));
 }catch(error){console.error('Textbook rename failed',error);return json({error:'教材名称未能保存，请重试。'},503);}
}
