...
return{answerActivity:completedAnswers.map(answer=>answer.answered_at),plannerScope:`account:${user}`,questions:questions.map((q,index)=>{const visible=s?.answers[q.id]&&(s.mode!=='exam'||s.status==='finished');return visible?q:unseenQuestions[index];}),session:s,sessions:rows.map(serial),questionStats:buildQuestionStudyStats(graded.map(answer=>({questionId:answer.question_id,selected:answer.selected,correct:!!answer.correct}))),stats:{answered:completedAnswers.length,correct,accuracy:completedAnswers.length?Math.round(correct/completedAnswers.length*100):null,wrongCount:mistakes.length,subjects:buildSubjectStudyStats(completedAnswers.map(answer=>({questionId:answer.question_id,correct:!!answer.correct})),questions)},mistakes};}
