...
export function AnswerReview({question:q,answer}:{question:Question;answer:{selected:string;correct?:boolean}}){
 const e=q.explanation,k=q.knowledge;
 if(!e)return null;
 if(e.kind==='publisher-original')return <OriginalAnswerReview question={q}/>;
 ...
 return <article id="answer-review" className="answer-review compact-review" aria-label="答案解析与知识点回顾">
  ...
  <div className="review-body">
   {e.supplementaryReview&&<details className="supplementary-review-sources"><summary>补充解析（非 SRA 官方） · 依据</summary><p>正确选项沿用 <a href={e.sourceUrl} target="_blank" rel="noopener noreferrer">SRA 官方答案表</a>。下方中文讲解由本站补充，依据核对日期：{e.supplementaryReview.reviewedAt}。</p><ul>{e.supplementaryReview.sources.map(source=><li key={source.url}><a href={source.url} target="_blank" rel="noopener noreferrer">{source.label}</a></li>)}</ul></details>}
   ...
   {e.textbookExample&&<details className="textbook-example"><summary>用教材例子理解 · {e.textbookExample.title}</summary><div><p><b>教材情境</b><Terms text={e.textbookExample.facts}/></p><p><b>为什么</b><Terms text={e.textbookExample.lesson}/></p><p><b>回到本题</b><Terms text={e.textbookExample.connection}/></p></div></details>}
