...
export function AnswerReview({question:q,answer,onOpenTextbook}:{question:Question;answer:{selected:string;correct?:boolean};onOpenTextbook:(ref:LinkedTextbookReference)=>void}){
 ...
 return <article id="answer-review" className="answer-review" aria-label="答案解析与知识点回顾">
  <h2 className="sr-only">答案与解析</h2>
  <div className="review-body">
   <nav className="review-jumps" aria-label="解析章节"><a href="#reasoning">为什么选 {e.answer}</a>{e.concepts?.length&&<a href="#related-concepts">概念与区别</a>}<a href="#option-review">逐项分析</a>{hasQuickNotes&&<a href="#knowledge-review"><BookOpen size={14}/>考点回顾</a>}{!!newglawNotes.length&&<a href="#newglaw-knowledge"><BookOpen size={14}/>新生代要点</a>}</nav>
   {chapter&&<div className="review-chapter"><span>第 {chapter.number} 章 · {chapter.zh}</span>{q.relatedChapterIds?.map(id=>{const c=chapterById(id);return c?<span key={id}>关联：第 {c.number} 章 {c.zh}</span>:null;})}</div>}
   {!!q.topicTags?.length&&<div className="review-topic-tags">{q.topicTags.map(tag=><span key={tag}><Terms text={tag} label/></span>)}</div>}
   <section id="reasoning" className="analysis-section answer-rationale"><div className="tested-topic">本题考查 · <Terms text={e.topic} label/></div><h3>为什么选 {e.answer}</h3><p><Terms text={e.zh}/></p>{e.reasoningSteps&&<ol className="reasoning-steps">{e.reasoningSteps.map(step=><li key={step.title}><h4><Terms text={step.title}/></h4><p><Terms text={step.detail}/></p></li>)}</ol>}</section>
   ...
   <section id="option-review" className="analysis-section"><h3>逐项分析</h3>{q.options.map(o=><div key={o.id} className={`option-analysis ${e.answer===o.id?'answer-option-correct':''}`}><strong className={e.answer===o.id?'option-key':''}>{o.id}</strong><div><span className="option-verdict">{e.answer===o.id?'正确':'不选'}{answer.selected===o.id?' · 你的选择':''}</span><p><Terms text={e.options[o.id].zh}/></p></div></div>)}</section>
   ...
   {!!e.textbookReferences?.length&&<details className="textbook-references"><summary>教材依据 · 章节与页码</summary><p>页码均为上传文件的 PDF 页序（从封面起算），不一定等于书内印刷页码。Revise 为主线，Notes 用于交叉核对。</p><ul>{e.textbookReferences.map((ref,i)=><li key={i}><strong>{ref.book}</strong><span>{ref.chapter} · {ref.pages.includes('PDF')?ref.pages:`PDF 第 ${ref.pages} 页`}</span><span>{ref.section}</span></li>)}</ul></details>}
  </div>
