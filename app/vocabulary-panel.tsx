...
export function VocabularyPanel({controller,onAdd,onEdit,onSource,onPractice,activeExam}:{controller:VocabularyController;onAdd:()=>void;onEdit:(card:VocabularyCard)=>void;onSource:(id:string)=>void;onPractice:()=>void;activeExam:boolean}){
 ...
 {activeExam?<div className="memory-empty"><ClockNotice/><p>限时模拟仍在计时，交卷后再复习生词。</p><Button variant="outline" onClick={onPractice}>返回答题</Button></div>:current?<div className="memory-card" key={token}>
...
function ClockNotice(){return <p className="memory-exam-label">限时模拟进行中</p>;}
