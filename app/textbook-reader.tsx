...
export function TextbookReader({references=[],initial,onClose,books,standalone=false,open=true,sourceQuestionId,annotations,bookmarks,onPositionChange,toolbarStart,navigationRequest,onToolbarHeightChange}:{references?:LinkedTextbookReference[];initial:{bookId:string;page:number};onClose?:()=>void;books?:Textbook[];standalone?:boolean;open?:boolean;sourceQuestionId?:string;annotations:TextbookAnnotationsController;bookmarks?:TextbookBookmarksController;onPositionChange?:(bookId:string,page:number)=>void;toolbarStart?:ReactNode;navigationRequest?:number;onToolbarHeightChange?:(height:number)=>void}){
 ...
 const content=<div className="textbook-reader-inner" onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget))setSelection(null);}}>
  ...
  <div ref={toolbarRef} className={`textbook-toolbar${standalone?' textbook-toolbar-compact':''}`}>
   {standalone&&<div className="textbook-toolbar-heading">{toolbarStart&&<div className="textbook-toolbar-start">{toolbarStart}</div>}<h2 className="textbook-toolbar-book" title={book.title}>{book.shortTitle}</h2></div>}
