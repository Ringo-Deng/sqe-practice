...
export function TextbookReader({references=[],initial,onClose,books,standalone=false,open=true,sourceQuestionId,annotations,onPositionChange}:{references?:LinkedTextbookReference[];initial:{bookId:string;page:number};onClose?:()=>void;books?:Textbook[];standalone?:boolean;open?:boolean;sourceQuestionId?:string;annotations:TextbookAnnotationsController;onPositionChange?:(bookId:string,page:number)=>void}){
 ...
 const onReaderKeyDown=(event:React.KeyboardEvent<HTMLDivElement>)=>{
  if(event.target!==event.currentTarget||document.activeElement!==event.currentTarget||event.defaultPrevented||event.altKey||event.ctrlKey||event.metaKey||event.shiftKey||event.nativeEvent.isComposing||editing||selection||!open)return;
 ...
 const content=<div className="textbook-reader-inner" onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget))setSelection(null);}}>
  ...
  <footer className="textbook-reader-footer"><span className="textbook-page-note-count"><Highlighter size={14}/>{pageAnnotations.length?`本页 ${pageAnnotations.length} 条高亮`:'选中文字即可高亮'}</span><div className="textbook-reader-actions">{canSaveTextbookLocally(book)&&(localState==='saved'?<span className="textbook-local-status"><HardDrive size={14}/>已保存到此浏览器</span>:<Button type="button" size="sm" variant="ghost" disabled={localState==='checking'||localState==='saving'} onClick={()=>void saveLocal()}>{localState==='saving'?<Loader2 className="animate-spin" size={14}/>:<Download size={14}/>} {localState==='saving'?'正在保存…':'保存到此浏览器'}</Button>)}{book.url&&<a href={`${book.url}#page=${page}`} target="_blank" rel="noreferrer">单独打开 PDF<ExternalLink size={14}/></a>}</div></footer>
