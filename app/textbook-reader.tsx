...
import {textbookById,textbookPageSource,type LinkedTextbookReference,type Textbook,type TextbookReaderTarget} from '@/lib/textbooks';
...
function ContinuousPdfPage({book,page,zoom,width,active,annotations,onSelect,onOpen}:{book:Textbook;page:number;zoom:number;width:number;active:boolean;annotations:TextbookAnnotation[];onSelect:(draft:SelectionDraft)=>void;onOpen:(item:TextbookAnnotation)=>void}){
 const pageUnit=book.pdfSlice?'段':'页';
 const host=useRef<HTMLDivElement>(null);
 const [error,setError]=useState(''),[busy,setBusy]=useState(active),[rendered,setRendered]=useState(false),[renderRevision,setRenderRevision]=useState(0),[retry,setRetry]=useState(0),[aspect,setAspect]=useState(book.pdfSlice?book.pdfSlice.width/Math.min(book.pdfSlice.height,book.pdfSlice.totalHeight-(page-1)*book.pdfSlice.height):210/297);
 useEffect(()=>{
 ...
 useEffect(()=>{
  ...
  const work=(async()=>{
   ...
   const base=pdfPage.getViewport({scale:1}),scale=width/base.width*zoom,viewport=pdfPage.getViewport({scale});
   const sliceTop=book.pdfSlice?(page-1)*book.pdfSlice.height*scale:0;
   const displayHeight=book.pdfSlice?Math.min(book.pdfSlice.height*scale,viewport.height-sliceTop):viewport.height;
   setAspect(viewport.width/displayHeight);
   const deviceRatio=Math.min(window.devicePixelRatio||1,2),maxRasterPixels=5_000_000;
   const desiredRatio=Math.max(1,Math.min(deviceRatio,Math.sqrt(maxRasterPixels/(viewport.width*displayHeight))));
   const makeCanvas=(pixelRatio:number)=>{const canvas=document.createElement('canvas');canvas.setAttribute('aria-label',`${book.shortTitle} · 第 ${page} ${pageUnit}`);canvas.width=Math.floor(viewport.width*pixelRatio);canvas.height=Math.floor(displayHeight*pixelRatio);canvas.style.width=`${viewport.width}px`;canvas.style.height=`${displayHeight}px`;return canvas;};
   const canvas=makeCanvas(1);
   const wrapper=document.createElement('div');wrapper.className='textbook-page';wrapper.lang='en';wrapper.style.width=`${viewport.width}px`;wrapper.style.height=`${displayHeight}px`;wrapper.style.setProperty('--scale-factor',String(scale));wrapper.style.setProperty('--total-scale-factor',String(scale));wrapper.appendChild(canvas);
   render=pdfPage.render({canvas,viewport,transform:sliceTop?[1,0,0,1,0,-sliceTop]:undefined});await render.promise;if(cancelled)return;
   ...
   if(desiredRatio>1.05)refineTimer=window.setTimeout(()=>{const idleWindow=window as IdleWindow;const sharpen=async()=>{if(cancelled)return;const sharpCanvas=makeCanvas(desiredRatio);try{refine=pdfPage.render({canvas:sharpCanvas,viewport,transform:[desiredRatio,0,0,desiredRatio,0,-sliceTop*desiredRatio]});await refine.promise;if(!cancelled)canvas.replaceWith(sharpCanvas);}catch(reason){if(!cancelled&&(!(reason instanceof Error)||reason.name!=='RenderingCancelledException'))console.warn('Textbook page refinement failed',reason);}};if(idleWindow.requestIdleCallback)refineIdle=idleWindow.requestIdleCallback(()=>void sharpen(),{timeout:1500});else void sharpen();},300);
   try{const lib=await loadPdfEngine();if(cancelled)return;const layer=document.createElement('div');layer.className='textLayer';if(sliceTop)layer.style.top=`-${sliceTop}px`;wrapper.insertBefore(layer,highlights);textLayer=new lib.TextLayer({textContentSource:pdfPage.streamTextContent(),container:layer,viewport});await textLayer.render();}catch{/* The PDF canvas remains readable if selectable text is unavailable. */}
 ...
 useEffect(()=>{
  ...
  for(const item of annotations)item.rects.forEach((rect,index)=>{const button=document.createElement('button');button.type='button';button.className='textbook-highlight';button.style.left=`${rect.x*100}%`;button.style.top=`${rect.y*100}%`;button.style.width=`${rect.width*100}%`;button.style.height=`${rect.height*100}%`;button.title=item.note||item.quote;button.setAttribute('aria-label',index===0?`查看第 ${page} ${pageUnit}高亮笔记`:'同一条高亮');button.addEventListener('click',event=>{event.stopPropagation();onOpen(item);});layer.appendChild(button);});
 },[annotations,onOpen,page,pageUnit,rendered,renderRevision]);
 ...
 return <section className="textbook-page-slot" data-textbook-page={page} aria-label={`第 ${page} ${pageUnit}`} onPointerUp={capture} onKeyUp={capture}>
  <div ref={host} className="textbook-page-host" style={rendered&&width?{width:width*zoom,height:width*zoom/aspect,overflow:'hidden'}:undefined}/>
  {!rendered&&<div className="textbook-page-placeholder" style={placeholderStyle}><div className="textbook-page-placeholder-status">{active&&busy?<><Loader2 size={18} className="animate-spin"/>正在打开第 {page} {pageUnit}…</>:error?<><span>{error}</span><Button variant="outline" onClick={()=>setRetry(n=>n+1)}><RotateCcw size={15}/>重试</Button></>:<span>第 {page} {pageUnit}</span>}</div></div>}
...
function AnnotationEditor({draft,bookTitle,onChange,onClose,controller,pageUnit='页'}:{draft:TextbookAnnotationDraft|null;bookTitle:string;onChange:(draft:TextbookAnnotationDraft)=>void;onClose:()=>void;controller:TextbookAnnotationsController;pageUnit?:string}){
...
export function TextbookReader({references=[],initial,onClose,books,standalone=false,open=true,sourceQuestionId,annotations,bookmarks,onPositionChange,toolbarStart,navigationRequest,onToolbarHeightChange,entryLocation}:{references?:LinkedTextbookReference[];initial:{bookId:string;page:number};onClose?:()=>void;books?:Textbook[];standalone?:boolean;open?:boolean;sourceQuestionId?:string;annotations:TextbookAnnotationsController;bookmarks?:TextbookBookmarksController;onPositionChange?:(bookId:string,page:number)=>void;toolbarStart?:ReactNode;navigationRequest?:number;onToolbarHeightChange?:(height:number)=>void;entryLocation?:TextbookReaderTarget}){
 ...
 const jumpToPage=useCallback((next:number,behavior:ScrollBehavior='smooth',offset=0)=>requestAnimationFrame(()=>{const root=scrollRoot.current,node=root?.querySelector<HTMLElement>(`[data-textbook-page="${next}"]`);if(root&&node)root.scrollTo({top:Math.max(0,node.offsetTop-8+node.clientHeight*offset),behavior});}),[]);
 ...
 useEffect(()=>{
  ...
  setBookId(initial.bookId);setPositions(current=>({...current,[initial.bookId]:initial.page}));setPageInput(String(initial.page));setSelection(null);requestAnimationFrame(()=>jumpToPage(initial.page,'auto',entryLocation?.page===initial.page?entryLocation.offset??0:0));
 },[initial.bookId,initial.page,jumpToPage,navigationRequest,entryLocation]);
 useLayoutEffect(()=>{
  const toolbar=toolbarRef.current;if(!standalone||!toolbar||!onToolbarHeightChange)return;
  const updateHeight=()=>onToolbarHeightChange(toolbar.offsetHeight);
  updateHeight();const observer=new ResizeObserver(updateHeight);observer.observe(toolbar);
  return()=>observer.disconnect();
 },[standalone,onToolbarHeightChange,book?.id]);
 ...
 const content=<div className="textbook-reader-inner" onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget))setSelection(null);}}>
  ...
  <div ref={toolbarRef} className={`textbook-toolbar${standalone?' textbook-toolbar-compact':''}`}>
   {standalone&&<div className="textbook-toolbar-heading">{toolbarStart&&<div className="textbook-toolbar-start">{toolbarStart}</div>}<h2 className="textbook-toolbar-book" title={entryLocation?.focusLabel?`${book.title} · 目标章节：${entryLocation.focusLabel}`:book.title}>{book.shortTitle}</h2></div>}
   <div className="textbook-toolbar-controls">
   <div className="textbook-pagination" role="group" aria-label="教材翻页">
    <Button size="icon" variant="outline" aria-label={book.pdfSlice?'导图上一段':'教材上一页'} title="上一页" disabled={page<=1} onClick={()=>go(page-1)}><ChevronLeft size={17}/></Button>
    <form onSubmit={event=>{event.preventDefault();go(Number(pageInput));}}><label className={standalone?'sr-only':undefined} htmlFor={`textbook-page-number-${standalone?'library':'question'}`}>PDF 页码</label><Input id={`textbook-page-number-${standalone?'library':'question'}`} aria-label={book.pdfSlice?'跳转导图分段，按回车确认':'跳转教材页码，按回车确认'} inputMode="numeric" value={pageInput} onFocus={event=>{pageInputFocused.current=true;event.target.select();}} onChange={event=>setPageInput(event.target.value)} onBlur={()=>{pageInputFocused.current=false;if(pageInput!==String(page))go(Number(pageInput));}} onKeyDown={event=>{if(event.key==='Escape'){event.preventDefault();setPageInput(String(page));}}}/><span className="textbook-page-total">/ {book.pageCount}{book.pdfSlice?' 段':''}</span></form>
    <Button size="icon" variant="outline" aria-label={book.pdfSlice?'导图下一段':'教材下一页'} title="下一页" disabled={page>=book.pageCount} onClick={()=>go(page+1)}><ChevronRight size={17}/></Button>
   </div>
   {!standalone&&<span className="textbook-scroll-hint">连续滚动阅读</span>}
   <div className="textbook-zoom" role="group" aria-label={book.pdfSlice?'导图缩放':'教材缩放'}><Button size="icon" variant="ghost" aria-label={book.pdfSlice?'缩小导图':'缩小教材'} disabled={zoom<=.75} onClick={()=>setZoom(value=>Math.max(.75,value-.25))}><ZoomOut size={16}/></Button><Button size="sm" variant="ghost" className="textbook-zoom-value" aria-label={`当前缩放 ${Math.round(zoom*100)}%，点击适应阅读区宽度`} title="适应阅读区宽度（100%）" onClick={()=>setZoom(DEFAULT_TEXTBOOK_ZOOM)}>{zoom===1&&!standalone?'100% · 适宽':`${Math.round(zoom*100)}%`}</Button><Button size="icon" variant="ghost" aria-label={book.pdfSlice?'放大导图':'放大教材'} disabled={zoom>=2} onClick={()=>setZoom(value=>Math.min(2,value+.25))}><ZoomIn size={16}/></Button></div>
   <div className="textbook-toolbar-tools" role="group" aria-label="教材操作">
   {bookmarks&&<Button type="button" size="sm" variant="ghost" className="textbook-bookmark-toggle" disabled={!bookmarks.data||bookmarks.loading||bookmarks.busy} aria-label={`${currentBookmark?'编辑':'添加'}第 ${page} ${book.pdfSlice?'段':'页'}书签`} title={currentBookmark?'编辑本页书签备注':'添加本页书签'} data-bookmarked={!!currentBookmark} onClick={()=>setEditingBookmark(currentBookmark?{...currentBookmark}:{bookId:book.id,page,note:''})}><Bookmark size={16} fill={currentBookmark?'currentColor':'none'}/><span>{currentBookmark?'编辑书签':'添加书签'}</span></Button>}
