...
export function TextbookReader({references=[],initial,onClose,books,standalone=false,sourceQuestionId,annotations,onPositionChange}:{references?:LinkedTextbookReference[];initial:{bookId:string;page:number};onClose?:()=>void;books?:Textbook[];standalone?:boolean;sourceQuestionId?:string;annotations:TextbookAnnotationsController;onPositionChange?:(bookId:string,page:number)=>void}){
 ...
 const content=<div className="textbook-reader-inner">
  ...
  <div className="textbook-pages textbook-pages-continuous" ref={scrollRoot} onScroll={syncVisiblePage} onPointerDown={()=>setSelection(null)}><div className="textbook-page-stack">{Array.from({length:book.pageCount},(_,index)=>{const number=index+1;return <ContinuousPdfPage key={`${book.id}:${number}`} book={book} page={number} zoom={zoom} width={readerWidth} active={Math.abs(number-page)<=2} annotations={annotationsByPage.get(number)??[]} onSelect={setSelection} onOpen={openAnnotation}/>;})}</div></div>
  <footer className="textbook-reader-footer"><span>{book.shortTitle} · 第 {page} / {book.pageCount} 页</span><span className="textbook-page-note-count"><Highlighter size={14}/>{pageAnnotations.length?`本页 ${pageAnnotations.length} 条高亮`:'选中文字即可高亮'}</span>{book.url&&<a href={`${book.url}#page=${page}`} target="_blank" rel="noreferrer">单独打开 PDF<ExternalLink size={14}/></a>}</footer>
 ...
 if(overlay)return <Dialog open onOpenChange={open=>{if(!open)onClose?.();}}><DialogContent className="textbook-dialog" showCloseButton={false}><DialogTitle className="sr-only">教材原文</DialogTitle><DialogDescription className="sr-only">在教材之间切换，连续滚动阅读，并对原文进行高亮或记录笔记。</DialogDescription>{content}</DialogContent></Dialog>;
 return standalone?<section className="textbook-reader textbook-reader-standalone" aria-label="教材原文阅读区">{content}</section>:<aside id="textbook-reader" className="textbook-reader" aria-label="教材原文阅读区">{content}</aside>;
}
