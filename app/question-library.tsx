...
import {ArrowRight,Check,ChevronRight,FolderOpen} from 'lucide-react';
import {Button} from '@/components/ui/button';
import {subjects,subjectById} from '@/lib/subjects';
import {questionSources,sourceById,questionNumberLabel,reviseAssessmentSessions} from '@/lib/question-sources';
...
export function QuestionLibrary({data,busy,onStart,onResume}:{data:StudyData;busy:boolean;onStart:(subjectId?:string,sourceId?:string,chapterId?:string,sourceSet?:string)=>void;onResume:(session:Session)=>void}){
 const[source,setSource]=useState('sra');
 ...
 const resume=exactSession(qs),sourceResume=exactSession(sourceQs);
 const count=(id:string)=>sourceQs.filter(q=>q.subjectId===id).length;
 const available=subjects.filter(s=>count(s.id)>0).length;
 ...
 return <div className="library-page">
  ...
  <section className="source-section" aria-label="题目来源">
   ...
   <div className="source-selection"><div className="source-summary"><p>{sourceQs.length} 题 · {available} 个科目</p>{source==='sra'&&<small>官方题目及答案 · 保留原始题号与命中率</small>}</div><div className="subject-actions">{sourceResume&&<Button variant="outline" disabled={busy} onClick={()=>onResume(sourceResume)}>继续本来源练习</Button>}<Button disabled={busy||!sourceQs.length} onClick={()=>onStart(undefined,sourceArg)}>练习全部 {sourceQs.length} 题<ArrowRight size={16}/></Button></div></div>
   {source==='revise'&&<div className="assessment-sessions">{reviseAssessmentSessions.map(set=>{const items=sourceQs.filter(q=>q.sourceSet===set.id),previous=exactSession(items);return <div className="assessment-session" key={set.id}><div><h3>{set.name}<span>{items.length} 题</span></h3><p>{set.edition}</p><small>{set.details}</small></div><div className="subject-actions">{previous&&<Button variant="outline" disabled={busy} onClick={()=>onResume(previous)}>继续练习</Button>}<Button variant="outline" disabled={busy||!items.length} onClick={()=>onStart(undefined,'revise',undefined,set.id)}>开始练习<ArrowRight size={15}/></Button></div></div>;})}</div>}
  ...
  {subject&&<section id="subject-detail" className="subject-detail" aria-live="polite"><div className="subject-detail-heading"><div><div className="eyebrow">{subject.group} / {sourceName}</div><h2>{subject.zh}<span>{qs.length} 题</span></h2></div>{qs.length>0&&<div className="subject-actions">{resume&&<Button disabled={busy} variant="outline" onClick={()=>onResume(resume)}>继续上次练习</Button>}<Button disabled={busy} onClick={()=>onStart(subject.id,sourceArg)}>开始本科练习<ArrowRight size={16}/></Button></div>}</div>
   <p className="subject-detail-scope">{subject.scope}</p>
   {qs.length>0?<>{questionSources.filter(s=>qs.some(q=>q.sourceId===s.id)).map(s=>{const sourceItems=qs.filter(q=>q.sourceId===s.id);const translated=sourceItems.filter(q=>q.stemZh).length;const language=translated===sourceItems.length?'中文译文':translated?'部分含中文译文':'英文原文';return <div className="collection-row" key={s.id}><span className="collection-icon"><FolderOpen size={21}/></span><div><h3>{s.name}</h3><p>{subject.en} · {sourceItems.length} 道单选题 · {s.id==='revise'?`原书解析 · ${language}`:translated?'中文解析 · 速记考点':'官方答案 · 英文原题'}</p></div></div>;})}<p className="collection-numbers">题号：{qs.map(questionNumberLabel).join('、')}</p></>:<div className="subject-empty-state"><FolderOpen size={27}/><div><h3>这个来源暂无本科题目</h3><p>可以切换来源，或上传对应科目的题库。</p></div></div>}
   {qs.some(q=>q.chapterId)&&!!chapterGroups.length&&<div className="chapter-section"><div className="chapter-heading"><h3>按章节练习</h3><p>按 Revise 教材目录 · 每题按主考点归入一章，关联考点见解析</p></div>{chapterGroups.map(group=><section className="chapter-book" key={group.bookId} aria-label={group.bookZh}><div className="chapter-book-heading"><h4>Revise · {group.bookZh}</h4><p lang="en">{group.bookEn}</p></div><div className="chapter-grid">{group.items.map(c=>{const items=filterQuestions(qs,{chapterId:c.id});const previous=exactSession(items);return <div className={`chapter-card ${items.length?'':'chapter-empty'}`} key={c.id}><span className="chapter-number">{String(c.number).padStart(2,'0')}</span><div className="chapter-copy"><h4>{c.zh}</h4><p lang="en">{c.en}</p><small>{items.length?`${items.filter(q=>practiced.has(q.id)).length} / ${items.length} 题已练`:'暂无题目'}</small></div><div className="chapter-actions">{previous&&<Button size="sm" variant="outline" disabled={busy} onClick={()=>onResume(previous)}>继续</Button>}<Button size="sm" variant="outline" disabled={busy||!items.length} onClick={()=>onStart(subject.id,sourceArg,c.id)} aria-label={`练习${group.bookZh}第${c.number}章${c.zh}`}>{items.length?`练习 ${items.length} 题`:'待导入'}</Button></div></div>;})}</div></section>)}</div>}
  </section>}
 </div>;
}
