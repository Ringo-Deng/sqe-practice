...
export function QuestionLibrary({data,busy,onStart,onResume}:{data:StudyData;busy:boolean;onStart:(subjectId?:string,sourceId?:string,chapterId?:string,sourceSet?:string)=>void;onResume:(session:Session)=>void}){
 ...
 return <div className="library-page">
  ...
  {subject&&<section id="subject-detail" className="subject-detail" aria-live="polite"><div className="subject-detail-heading"><div><div className="eyebrow">{subject.group} / {sourceName}</div><h2>{subject.zh}<span>{qs.length} 题</span></h2></div>{qs.length>0&&<div className="subject-actions">{resume&&<Button disabled={busy} variant="outline" onClick={()=>onResume(resume)}>继续上次练习</Button>}<Button disabled={busy} onClick={()=>onStart(subject.id,sourceArg)}>开始本科练习<ArrowRight size={16}/></Button></div>}</div>
   {qs.some(q=>q.chapterId)&&!!chapterGroups.length&&<div className="chapter-section compact-chapter-section">{chapterGroups.map(group=><section className="chapter-book" key={group.bookId} aria-label={group.bookZh}><div className="chapter-grid">{group.items.map(c=>{const items=filterQuestions(qs,{chapterId:c.id});const previous=exactSession(items);return <div className={`chapter-card ${items.length?'':'chapter-empty'}`} key={c.id}><div className="chapter-info"><span className="chapter-number">{String(c.number).padStart(2,'0')}</span><div className="chapter-copy"><h4>{c.zh}</h4><p lang="en">{c.en}</p></div></div><div className="chapter-actions"><div className="chapter-action-buttons">{previous&&<Button size="sm" variant="outline" disabled={busy} onClick={()=>onResume(previous)}>继续</Button>}<Button size="sm" variant="outline" disabled={busy||!items.length} onClick={()=>onStart(subject.id,sourceArg,c.id)} aria-label={`练习${group.bookZh}第${c.number}章${c.zh}`}>{items.length?`练习 ${items.length} 题`:'待导入'}</Button></div><small className="chapter-progress">{items.length?`${items.filter(q=>practiced.has(q.id)).length} / ${items.length} 题已练`:'暂无题目'}</small></div></div>;})}</div></section>)}</div>}
  </section>}
 </div>;
}
