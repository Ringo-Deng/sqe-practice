...
type Props={
 ...
 onShowCorrected:(show:boolean)=>void;
 onRetry:(questionId?:string,subjectId?:string)=>void;
 onPractice:()=>void;
};

export function MistakeNotebook({data,busy,showCorrected,onShowCorrected,onRetry,onPractice}:Props){
 const pending=data.mistakes.filter(mistake=>!mistake.lastCorrect);
 const visible=showCorrected?data.mistakes:pending;
 ...
 return <div className="mistake-notebook">
  <div className="mistake-top">
   <Tabs value={showCorrected?'all':'pending'} onValueChange={value=>onShowCorrected(value==='all')}>
    <TabsList><TabsTrigger value="pending">待巩固 {pending.length}</TabsTrigger><TabsTrigger value="all">全部错题 {data.mistakes.length}</TabsTrigger></TabsList>
   </Tabs>
   <Button disabled={!pending.length||busy} onClick={()=>onRetry()}><RotateCcw size={15}/>重练全部</Button>
  </div>
  {!visible.length?<div className="empty">
   <CheckCheck size={34}/>
   <h2>{data.stats.answered?'暂时没有待巩固的错题':'你的错题会出现在这里'}</h2>
   <p>{data.stats.answered?'重做答对的题目仍保留在“全部错题”中。':'先做几道题，答错的题目会自动收录。'}</p>
   ...
   {categories.map(subject=>{
    ...
    const pendingCount=entries.filter(({mistake})=>!mistake.lastCorrect).length;
    return <details key={subject.id} className="mistake-subject">
     <summary>
      <h2>{subject.zh}</h2>
      <span>{entries.length} 题{showCorrected?` · ${pendingCount} 待巩固`:''}</span>
      <ChevronRight size={18} className="mistake-subject-chevron"/>
     </summary>
     <div className="mistake-subject-body">
      {subject.id!=='other'&&pendingCount>0&&<div className="mistake-subject-actions"><Button variant="outline" size="sm" disabled={busy} onClick={()=>onRetry(undefined,subject.id)}><RotateCcw size={14}/>重练本科目（{pendingCount}）</Button></div>}
      {entries.map(({mistake,question})=><article className="mistake-item" key={mistake.questionId}>
       <div className="mistake-item-meta"><span>错过 {mistake.wrongCount} 次</span><span className={mistake.lastCorrect?'mistake-corrected':'mistake-pending'}>{mistake.lastCorrect?'已订正':'待巩固'}</span></div>
       <h3>{mistake.topic}</h3>
       <p>{question?normalizeInlineQuestionText(question.stem):'题目暂不可用'}</p>
       {!mistake.lastCorrect&&<Button variant="outline" size="sm" disabled={busy||!question} onClick={()=>onRetry(mistake.questionId)}>重做这道题<ArrowRight size={15}/></Button>}
      </article>)}
     </div>
    </details>;
   })}
