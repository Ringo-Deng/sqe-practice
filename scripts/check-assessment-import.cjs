...
const originalQlts=[...require('../lib/qlts-mock-exams-1-5.json'),...require('../lib/qlts-mock-exams-6-10.json'),...require('../lib/qlts-mock-exams-11-15.json')];
...
async function main(){
 ...
 assert.equal(importedQlts.length,1190);
 ...
 data=await post({action:'start',id:qltsPracticeId,mode:'practice',sourceId:'qlts',sourceSet:'qlts-mock-exam-11'});
 assert.equal(data.session.questionIds.length,72);
 const qltsFirst=importedQlts.find(q=>q.sourceSession===11);
 ...
 data=await post({action:'start',id:qltsExamId,mode:'exam',sourceId:'qlts',sourceSet:'qlts-mock-exam-15'});
 assert.equal(data.session.questionIds.length,72);
 const qltsExamFirst=importedQlts.find(q=>q.sourceSession===15);
 ...
 console.log('Passed: SRA and Revise regressions plus fifteen bilingual QLTS mock sets (1,190 current-law-screened questions), answer keys, scoring, exam answer protection, invalid set rejection and existing study history.');
 sqlite.close();
}
