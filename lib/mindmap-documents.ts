...
const subjects:Record<string,string>={ethics:'legal-services',business:'business','legal-system':'legal-system',contract:'contract',dispute:'dispute',tort:'tort','criminal-law':'criminal-liability','criminal-practice':'criminal-practice','land-property':'land',trusts:'trusts',wills:'wills',taxation:'business'};
export const mindMapBooks:Textbook[]=catalog.map(map=>({
 id:`mindmap-${map.id}`,title:`思维导图 · ${map.title}`,shortTitle:`导图 · ${map.title}`,subjectId:subjects[map.id],
